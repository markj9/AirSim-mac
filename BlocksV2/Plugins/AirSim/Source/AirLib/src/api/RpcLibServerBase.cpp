...
namespace airlib
{
        ...
        : api_provider_(api_provider)
    {
        ...
        pimpl_->server.bind("simGetCameraStream", [&](const std::string& ip_addr, int port, const std::string& camera_name, ImageCaptureBase::ImageType type, const std::string& vehicle_name, bool external) -> string {
            return getWorldSimApi()->getCameraStream(ip_addr, port, type, CameraDetails(camera_name, vehicle_name, external));
        });
