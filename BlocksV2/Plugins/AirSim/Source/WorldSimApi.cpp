...
std::string WorldSimApi::getCameraStream(const std::string& ip_addr, int port, ImageCaptureBase::ImageType image_type, const CameraDetails& camera_details)
{
    const ImageCaptureBase::ImageRequest request = ImageCaptureBase::ImageRequest(camera_details.camera_name, image_type, false, false);
    const UnrealImageCapture* camera = simmode_->getImageCapture(camera_details.vehicle_name, camera_details.external);
    simmode_->CameraDirector->connectCameraStream(ip_addr,  port, camera, request);
