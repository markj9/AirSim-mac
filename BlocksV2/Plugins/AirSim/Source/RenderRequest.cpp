...
RenderRequest::RenderRequest(TWeakObjectPtr<UGameViewportClient> game_viewport, std::function<void()>&& query_camera_pose_cb)
    : params_(nullptr), results_(nullptr), req_size_(0), wait_signal_(new msr::airlib::WorkerThreadSignal), game_viewport_(game_viewport),
    query_camera_pose_cb_(query_camera_pose_cb)
{
}
    ...
    bool async)
{
    ...
    if (use_safe_method) {
        ...
        compressRenderResults(params, results);
    }
    else {
        ...
        AsyncTask(ENamedThreads::GameThread, [this]() {
            ...
            for (unsigned int i = 0; i < req_size_; ++i) {
                // params_[i]->render_component->DetailMode = DM_High;
...
void RenderRequest::compressRenderResults(std::shared_ptr<RenderParams> params[], const std::vector<std::shared_ptr<RenderResult>>& results)
{
...
{
    const FTextureRHIRef& TextureRef = rt_resource->GetRenderTargetTexture();
    
    // Store the address of that reference in a pointer
    const FTextureRHIRef* TextureRHIPtr = &TextureRef;
    
    size = rt_resource->GetSizeXY();
    result->width = size.X;
    result->height = size.Y;
    result->render_target = TextureRHIPtr;
