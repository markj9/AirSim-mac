                                ...
                                bool async) const
{
    if (cameras_->valsSize() == 0) {
        ...
        getSceneCaptureImage(requests, responses, use_safe_method, async);
}
                                              ...
                                              bool async) const
{
    ...
    const UnrealImageCapture* This = this;
    auto query_camera_pose_cb = [This]()   {

        // size_t count = requests.size();
        // for (size_t i = 0; i < count; i++) {
        //     const ImageRequest& request = requests.at(i);
        //     APIPCamera* camera = This->cameras_->at(request.camera_name);
        //     ImageResponse& response = responses.at(i);
    ...
    RenderRequest render_request{ gameViewport, query_camera_pose_cb };

    render_request.getScreenshot(
        render_params.data(),
        render_results,
        render_params.size(),
        use_safe_method,
    ...
    for (unsigned int i = 0; i < requests.size(); ++i) {
        ...
        response.render_target = render_results[i]->render_target;
