...
public:
    ...
    struct RenderResult
    {
        const FTextureRHIRef* render_target;
...
private:
    ...
    static void compressRenderResults(std::shared_ptr<RenderParams> params[],
        const std::vector<std::shared_ptr<RenderResult>>& results);
    
    std::shared_ptr<RenderParams>* params_;
    std::shared_ptr<RenderResult>* results_;
    std::function<void(std::vector<std::shared_ptr<RenderResult>>&)> callback_;
    unsigned int req_size_;

    std::shared_ptr<msr::airlib::WorkerThreadSignal> wait_signal_;

    bool saved_DisableWorldRendering_ = false;
    TWeakObjectPtr<UGameViewportClient> const game_viewport_;
    FDelegateHandle end_draw_handle_;
    TFunctionRef<void()> query_camera_pose_cb_;

public:
    RenderRequest(TWeakObjectPtr<UGameViewportClient> game_viewport, std::function<void()>&& query_camera_pose_cb);
    ~RenderRequest();
