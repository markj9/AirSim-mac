...
public class AirSim : ModuleRules
{
    ...
    public AirSim(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        PublicDependencyModuleNames.AddRange(new string[] { "Core", "CoreUObject", "Engine", "InputCore",
            ...
            "Networking",
            "DeveloperSettings",
            "ImageWrapper", "RenderCore", "RHI", "PhysicsCore", "AssetRegistry", "ChaosVehicles", "Landscape", "CinematicCamera" });
        ...
        // Add CUDA include path (adjust the path as needed)
        PublicIncludePaths.Add(Path.Combine(ModuleDirectory, "ThirdParty/FFmpeg/mac/include"));

        
        PublicIncludePaths.Add(Path.Combine(AirLibPath, "include"));
        PublicIncludePaths.Add(Path.Combine(AirLibPath, "deps", "eigen3"));
        
        string FFmpegBinariesPath = Path.Combine(ModuleDirectory, "ThirdParty/FFmpeg/mac/lib");
        
        // Copy each DLL to the output folder.
        RuntimeDependencies.Add("$(BinaryOutputDir)/libavcodec.61.dylib", Path.Combine(FFmpegBinariesPath, "libavcodec.61.dylib"));
        RuntimeDependencies.Add("$(BinaryOutputDir)/libavutil.59.dylib", Path.Combine(FFmpegBinariesPath, "libavutil.59.dylib"));
        RuntimeDependencies.Add("$(BinaryOutputDir)/libavformat.59.dylib", Path.Combine(FFmpegBinariesPath, "libavformat.61.dylib"));
        RuntimeDependencies.Add("$(BinaryOutputDir)/libswscale.8.dyli", Path.Combine(FFmpegBinariesPath, "libswscale.8.dylib"));
        RuntimeDependencies.Add("$(BinaryOutputDir)/libswresample.5.dylib", Path.Combine(FFmpegBinariesPath, "libswresample.5.dylib"));
    ...
    private void AddOSLibDependencies(ReadOnlyTargetRules Target)
    {
        ...
        PublicAdditionalLibraries.Add(Path.Combine(ModuleDirectory, "ThirdParty/FFmpeg/mac/lib/libavcodec.61.dylib"));
        PublicAdditionalLibraries.Add(Path.Combine(ModuleDirectory, "ThirdParty/FFmpeg/mac/lib/libavutil.59.dylib"));
        PublicAdditionalLibraries.Add(Path.Combine(ModuleDirectory, "ThirdParty/FFmpeg/mac/lib/libavformat.61.dylib"));
        PublicAdditionalLibraries.Add(Path.Combine(ModuleDirectory, "ThirdParty/FFmpeg/mac/lib/libswresample.5.dylib"));
        PublicAdditionalLibraries.Add(Path.Combine(ModuleDirectory, "ThirdParty/FFmpeg/mac/lib/libswscale.8.dylib"));
    }
